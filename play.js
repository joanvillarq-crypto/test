...
const Trailerfunc = function (id) {
  return `<iframe name="framez" id="framez" class="youtubePlayer" src="https://flicky.host/embed/tv/${id}/1/1?autoPlay=true&title=true&poster=true&mute=false&nextbutton=true&sandbox=true&server=OYO" gesture="media" width="100%" height="100%" loading="lazy" frameborder="0" allowfullscreen></iframe>`;
...
const recommTvshowFun = (mov) => {
  let url = "./play.html?id=" + encodeURIComponent(mov.id);
...
const simimarTvshowfun = (movie) => {
  let url = "./play.html?id=" + encodeURIComponent(movie.id);
