...
<h2 class="episode_title"> <span class="episodenumm">${
  ...
  }.</span> ${curre.name} - ${fetcid} - ${curre.season_number} - ${curre.episode_number}</h2>
<p class="episode_description">${curre.overview}</p>
</div>
<div id="${curre.episode_number}" class="tvOpen mfp-hide">
  <iframe src="https://freembed.site/embed/tv/?id=${fetcid}&season=${curre.season_number}&episode=${curre.episode_number}" style="position:fixed;bottom:0;width:320px;height:200px;border:none;z-index:999 !important;" allowfullscreen></iframe>
</div>`;

$(function() {
  $('.tvOpen1').magnificPopup({
   type:'inline',
   closeBtnInside: true,
   midClick: true
  });
...
const unreleasedEpisodev = function (curre) {
  return `<a class="unreleased_link" href=" https://freembed.site/embed/tv?id=${fetcid}&s=${
    curre.season_number
